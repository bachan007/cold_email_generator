...
def main():
    ...
    # Upload resume file
    resume_file = st.file_uploader("Upload the resume (PDF format)", type="pdf")
    company_url = st.text_input("Enter the company website URL")

    if resume_file:
        # Display file type and size for debugging
        st.write(f"File type: {resume_file.type}")
        st.write(f"File size: {resume_file.size} bytes")

        # Save the uploaded file to a temporary location
        with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp_file:
            tmp_file.write(resume_file.read())
            tmp_file_path = tmp_file.name
        st.write("Resume file saved successfully.")
        ...
        if company_url:
            ...
            if st.button("Generate Email"):
                try:
                    ...
                    st.write("Extracting Company information.......")
                    ...
                    **Company Description:**
                    {company_info}
                    **Email Objective:**
                    - The goal of this email is to introduce candidate_name with his designation, highlight their qualifications, their most recent profile overview, and express interest in potential job opportunities that match with their recent projects and tech stack at company_name.
                    **Desired Outcome:**
            ...
            st.warning("Please enter the company URL before generating the email.")
    else:
